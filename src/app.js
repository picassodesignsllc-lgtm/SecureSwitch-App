...
function Dashboard() {
  return h('main', { className: 'dashboard', id: 'dashboard' },
    h('div', { className: 'main-column' },
      h(TopActions),
      h(Hero),
      h(Shortcuts),
      h('div', { className: 'lower-grid' }, h(Accounts), h(Activity))
    ),
    h('aside', { className: 'dashboard-side' }, h(ProtectionScore), h(ProtectedStatus), h(QuickActions), h(Readiness))
  );
}

function PageHeader({ title, description }) {
  return h('header', { className: 'page-header glass' }, h('div', null, h('p', { className: 'eyebrow' }, 'SecureSwitch Workspace'), h('h1', null, title), h('p', null, description)), h('a', { className: 'primary', href: '#dashboard' }, 'Back to Dashboard'));
}

function FeaturePage() {
  const route = currentRoute();
  const pages = {
    accounts: ['Accounts', 'Add, edit, delete, search, and filter the recovery accounts that power your SecureSwitch score.', [h(Accounts), h(AccountForm)]],
    switch: ['Switch Mode', 'Generate a checklist when you change numbers, devices, or recovery methods.', [h(SwitchMode)]],
    blackout: ['Blackout Mode', 'Prepare an emergency lockdown checklist for stolen devices, account compromise, and fraud.', [h(BlackoutMode), h(RecoveryWizardMVP)]],
    kit: ['Emergency Kit', 'Keep trusted contacts, offline backup codes, recovery letters, and critical notes ready.', [h(EmergencyKit)]],
    lookup: ['Recovery Lookup', 'Find every account connected to a recovery email, phone number, or identity handle.', [h(RecoveryLookup)]],
    settings: ['Settings', 'Manage workspace preferences, authentication, encrypted cloud sync, and vault security.', [h(Settings), h(SyncAndAuthPanel)]],
  };
  const [title, description, sections] = pages[route] || ['SecureSwitch', 'This recovery workspace is ready to be connected to the dashboard.', [h('section', { className: 'panel glass' }, h('h2', null, 'Coming soon'), h('p', null, 'This page will use the same premium SecureSwitch shell.'))]];
  return h('main', { className: 'page-view' }, h(PageHeader, { title, description }), ...sections);
}

function RoutedContent() {
  return currentRoute() === 'dashboard' ? h(Dashboard) : h(FeaturePage);
}
